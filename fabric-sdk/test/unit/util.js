...
function getSubmitter(username, password, client, t, loadFromConfig, peerId, mspid) {
	var caName, caUrl;
	for (let key in ORGS.cas){
		caName = 'fabric-ca-' + peerId;
		if (ORGS.cas[key].name === caName){
...
module.exports.getSubmitter = function(client, test, loadFromConfig, peerId) {
	if (arguments.length < 2) throw new Error('"client" and "test" are both required parameters');

	var fromConfig, mspid, id, caName, enrollSecret;
    ...
    for (let key in ORGS.cas){
        caName = 'fabric-ca-' + id;
        if (ORGS.cas[key].name === caName){
            enrollSecret = ORGS.cas[key].users_clients[0].enrollSecret;
            mspid = ORGS.cas[key].msp_id;
        }
    }

	return getSubmitter('admin', enrollSecret, client, test, fromConfig, id, mspid);
};
